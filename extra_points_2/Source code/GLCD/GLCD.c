...
void LCD_DrawGameVBorders(uint16_t x0, uint16_t y0, uint16_t y1, uint16_t x2, int thickness, uint16_t color){
	...
	for (i=0; i<y1-y0; i++){
		...
		if (i==32){
			LCD_DrawGameHBorder(paddle_x_p2,32,paddle_x_p2+39,10,Green); //paddle
		}
		if (i==278){
			LCD_DrawGameHBorder(paddle_x_p1,278,paddle_x_p1+39,10,Green); //paddle
		}
		if (i==156){
...
void PutChar_Reverse( uint16_t Xpos, uint16_t Ypos, uint8_t ASCI, uint16_t charColor, uint16_t bkColor )
{
    ...
    for( i=0; i<16; i++ )
    {
        ...
        for( j=0; j<8; j++ )
        {
            if( ((tmp_char >> (7 - j)) & 0x01) == 0x01 )
            {
                LCD_SetPoint( Xpos +7 - j, Ypos +15- i, charColor );  /* 字符颜色 */
            }
            else
            {
                LCD_SetPoint( Xpos +7- j, Ypos +15- i, bkColor );  /* 背景颜色 */
            }
        }
    }
}
